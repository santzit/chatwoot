...
# ---------------------------------------------------------------------------
write_infra_env() {
  local pg_user="$1" pg_pass="$2" redis_pass="$3" acme_email="$4"
...
# Let's Encrypt / ACME — TLS certificate registration
# Your real e-mail address. MUST NOT contain "example.com".
...
EOF
  ...
  write_infra_env "$pg_user" "$pg_pass" "$redis_pass" "$acme_email"
  success "Written .env"
...
start_stack() {
  # ------------------------------------------------------------------
  # Preflight check 1: ACME e-mail must be set in the root .env and
  # must not be a placeholder.  Let's Encrypt rejects registrations
  # for addresses at example.com with:
  #   "contact email has forbidden domain "example.com""
  # ------------------------------------------------------------------
  local acme_email_check
  acme_email_check=$(grep -E '^ACME_EMAIL=' .env 2>/dev/null | sed 's/^ACME_EMAIL=//')
  if [[ -z "$acme_email_check" ]]; then
    error "Root .env is missing ACME_EMAIL. Open .env and set your real e-mail address."
    error "  Example:  ACME_EMAIL=you@yourdomain.com"
    return 1
  fi
  if [[ "$acme_email_check" == *"example.com"* ]]; then
    error "ACME_EMAIL in .env is still set to a placeholder ('${acme_email_check}')."
    error "Open .env and replace it with your real e-mail address:"
    error "  ACME_EMAIL=you@yourdomain.com"
    return 1
  fi
