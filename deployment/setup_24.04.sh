...
#  4. Prompts for shared secrets and per-tenant domains, then writes:
#       infra/.env              – Postgres + Redis credentials
#       empresa{N}/.env         – per-tenant Chatwoot config
#  5. Patches the ACME e-mail in infra/traefik/traefik.yml.
#  6. Creates infra/traefik/acme.json with the required 0600 permissions.
#  7. Optionally starts infra then each tenant, and runs DB migrations.
#
# Usage
# -----
...
# ---------------------------------------------------------------------------
# Write infra/.env  (shared infra secrets)
# ---------------------------------------------------------------------------
write_infra_env() {
  ...
  cat > infra/.env <<EOF
# =============================================================================
# infra/.env  –  Shared infrastructure secrets (auto-generated by setup_24.04.sh)
...
write_tenant_env() {
  ...
  local domain="$2"      # empresa1.chat.example.com
  local pg_user="$3"
  local pg_pass="$4"
  local redis_pass="$5"
  local smtp_host="$6"
...
EOF
  ...
  sed -i "s/CHANGE_ME@example.com/${acme_email}/" infra/traefik/traefik.yml
  ...
  echo ""
  info "Enter the public domain for each Chatwoot tenant."
  info "DNS A records for these domains must point to this server's IP."
  echo ""
  ...
  # --- Write files -------------------------------------------------------
  info "Writing infra/.env and tenant env files…"

  write_infra_env "$pg_user" "$pg_pass" "$redis_pass"
  success "Written infra/.env"
...
prepare_traefik() {
  ...
  success "infra/traefik/acme.json created with mode 0600."
}

# ---------------------------------------------------------------------------
# Wait for infra healthchecks to pass
# ---------------------------------------------------------------------------
wait_for_infra() {
  local timeout=120
  local elapsed=0
  ...
  while [ $elapsed -lt $timeout ]; do
    local unhealthy
    unhealthy=$(docker compose --project-directory infra ps -q 2>/dev/null \
  ...
  if ! $all_healthy; then
    ...
    warn "Run 'docker compose --project-directory infra ps' to investigate."
  else
...
# ---------------------------------------------------------------------------
# Start the stack (infra first, then each tenant)
# ---------------------------------------------------------------------------
start_stack() {
  local answer
  ...
  info "Starting infra services (Traefik, Postgres, Redis)…"
...
print_summary() {
  ...
  for slug in "${tenants[@]}"; do
    if [[ -f "${slug}/.env" ]]; then
      local domain
      domain=$(grep -E '^DOMAIN=' "${slug}/.env" | cut -d= -f2)
      instance_urls="${instance_urls}    • https://${domain}  (${slug})\n"
    fi
  done
...
$(echo -e "$instance_urls")
  Manage infra:
    ...
    docker compose --project-directory infra logs -f
  ...
  Add a new tenant:
    ...
    # Edit empresa4/.env: TENANT_SLUG, DOMAIN, FRONTEND_URL,
    #   POSTGRES_DATABASE, SECRET_KEY_BASE, encryption keys
  ...
  Upgrade all:
    for dir in infra empresa1 empresa2 empresa3; do
      docker compose --project-directory "\$dir" pull
      docker compose --project-directory "\$dir" up -d
    done
    # Then run migrations for each tenant.

  Secrets:
    infra/.env          (shared Postgres + Redis credentials)
    empresa*/.env       (per-tenant secrets)

  Full setup log: ${LOG_FILE}

${YELLOW}⚠  Back up infra/.env and empresa*/.env — losing them means losing access to your data.${RESET}
EOF
}
